package com.robot.app.task.tc
{
   ...
   public class TaskClass
   {
      ...
      public function TaskClass(param1:NoviceFinishInfo)
      {
         ...
         this._noshow = [85,86,87,88,457,458,911,942,1165,1449];
         this._arrayItem = [];
         super();
         this._info = param1;
      ...
      private function getItem(param1:Array) : void
      {
         ...
         else
         {
            ...
            else if(!CountermarkXMLInfo.isminMark(id) && ItemXMLInfo.getCatID(id) == 14)
            {
               Alarm.show("你获得了" + TextFormatUtil.getRedTxt(count.toString()) + "个" + ItemXMLInfo.getName(id),this.loop);
            }
            else if(CountermarkXMLInfo.isminMark(id))
            {
               ItemInBagAlert.show(id,"你获得了" + count + "个<font color=\'#FF0000\'>" + CountermarkXMLInfo.getDes(id) + "</font>！",this.loop);
            }
            else
            {
               str = "";
