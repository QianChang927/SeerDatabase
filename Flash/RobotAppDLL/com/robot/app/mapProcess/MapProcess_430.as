package com.robot.app.mapProcess
{
   ...
   public class MapProcess_430 extends BaseMapProcess
   {
      ...
      {
         FightBossController.fightBoss("亚伦斯");
      }
      ...
      private function onCrystalClick(param1:MouseEvent) : void
      {
         ...
         if(_loc3_)
         {
            if(_loc3_.level >= 26 && TasksManager.getTaskStatus(322) == TasksManager.COMPLETE)
            {
               SocketConnection.addCmdListener(CommandID.PET_EVOLVTION,this.evo);
               SocketConnection.send(CommandID.PET_EVOLVTION,_loc3_.catchTime,1);
            }
            else
            {
               this.clickGoOn();
         ...
         if(info.userID == MainManager.actorID)
         {
            if(this._stone.hitTestPoint(info.endPos.x,info.endPos.y))
            {
               ...
               if(this._hitNum == 2)
               {
                  ...
                  ToolTipManager.add(this._stone,"活力浮石");
                  NpcDialog.show(NPC.SEER,["成功了！！接下来要怎么做呢？让我想想..."],null,null,false,function():void
                  {
                     NpcDialog.show(NPC.SEER,["有了！0xff0000用碎石块压住其他两支水柱，就可以将中央的水柱升起带动浮石，做出道路了0xffffff！！哎呀，我真是太聪明了！"],null,null,false,function():void
                     {
