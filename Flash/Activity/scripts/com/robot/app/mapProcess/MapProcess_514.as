package com.robot.app.mapProcess
{
   ...
   public class MapProcess_514 extends BaseMapProcess
   {
      ...
      private const PATH_STR:String = "resource/music/";
      ...
      private function playSound(param1:String) : void
      {
         ...
         this._sound.load(new URLRequest(this.PATH_STR + param1 + ".mp3"));
      ...
      override protected function init() : void
      {
         ...
         SocketConnection.addCmdListener(CommandID.NONO_ACTIVE_EXP,this.onGetExpHandler);
         SocketConnection.addCmdListener(CommandID.NONO_ACTIVE_ITEM,this.onGetItemHandler);
         ...
         this.seerBloodBar = new SpriteBloodBar(MapManager.currentMap.libManager.getMovieClip("BloodBar"),0,-80);
         this.hp = 200;
         this.maxHp = 200;
         ...
         while(_loc1_ < 6)
         {
            conLevel["music_" + _loc1_].addEventListener(MouseEvent.CLICK,this.onMusicMCFrameHandler);
            conLevel["music_" + _loc1_].buttonMode = true;
            _loc1_++;
         }
         var _loc2_:uint = 0;
         while(_loc2_ < 3)
         {
            _loc3_ = Number(NumberUtils.random(0,2));
            conLevel["pillarmc_" + _loc2_].x = this.pointArr[_loc2_][_loc3_].x;
            conLevel["pillarmc_" + _loc2_].y = this.pointArr[_loc2_][_loc3_].y;
            conLevel["pillarmc_" + _loc2_].addEventListener(Event.ENTER_FRAME,this.onPillarMCFrameHandler);
            _loc2_++;
         }
      ...
      private function pillarMCHitTest() : void
      {
         ...
         while(_loc1_ < 3)
         {
            _loc2_ = this.seerMC.localToGlobal(new Point());
            if(conLevel["pillarmc_" + _loc1_]["mc"])
            {
               _loc3_ = conLevel["pillarmc_" + _loc1_]["mc"].localToGlobal(new Point());
               if(Point.distance(_loc2_,_loc3_) < 30)
               {
                  if(!this.isHitPillar)
                  {
      ...
      private function pearHitTest() : void
      {
         ...
         if(_loc3_)
         {
            _loc2_ = _loc3_["mc"].localToGlobal(new Point());
         }
         if(Point.distance(_loc1_,_loc2_) < 130)
         {
            if(!this.isHitPear)
            {
               this.isHitPear = true;
      ...
      private function onGetExpHandler(param1:SocketEvent) : void
      {
         ...
         var _str:String = String(TextFormatUtil.getRedTxt(String(exp)));
         if(MainManager.actorInfo.superNono)
         {
            mc = MapManager.currentMap.libManager.getMovieClip("SuperAlarm");
      ...
      {
         var _loc2_:GetItemInfo = param1.data as GetItemInfo;
         var _loc3_:Array = _loc2_.itemArr;
         this.showAwardItem(_loc3_);
      }
      ...
      private function controlBloodBar(param1:int = 0, param2:int = 1, param3:Boolean = false) : void
      {
         ...
         if(param3)
         {
            ...
            if((_loc5_ = this.currentHitMC.localToGlobal(new Point(0,0))).x > 600)
            {
      ...
      private function onAimatHandler(param1:AimatEvent) : void
      {
         ...
         while(i < 3)
         {
            ...
            if(this.isHitedPillar["mc"])
            {
               if(this.isHitedPillar["mc"]["hit_mc"])
               {
                  if(this.isHitedPillar["mc"]["hit_mc"].hitTestPoint(p.x,p.y))
                  {
                     this.playSound("hit");
                     if(this.tempPillar)
                     {
                     ...
                     if(MainManager.actorInfo.superNono)
                     {
                        if(this.hitCount >= 3)
                        {
                           ...
                           {
                              clearTimeout(t2);
                              SocketConnection.send(CommandID.NONO_ACTIVE_EXP,2);
                           },300);
                        }
                     }
         ...
         if(this.switchMC)
         {
            if(this.switchMC.hitTestPoint(p.x,p.y))
            {
               ...
               if(MainManager.actorInfo.superNono)
               {
                  if(this.hitCount >= 1)
                  {
                     SocketConnection.send(CommandID.NONO_ACTIVE_EXP,1);
                     this.hitCount = 0;
                     DisplayUtil.removeForParent(this.bloodBar);
                     this.bloodBar = null;
         ...
         if(this.bigBossMC)
         {
            if(this.bigBossMC.hitTestPoint(p.x,p.y))
            {
               ...
               if(MainManager.actorInfo.superNono)
               {
                  if(this.hitCount >= 5)
                  {
                     SocketConnection.send(CommandID.NONO_ACTIVE_EXP,3);
                     this.hitCount = 0;
                     DisplayUtil.removeForParent(this.bloodBar);
                     this.bloodBar = null;
      ...
      override public function destroy() : void
      {
         ...
         SocketConnection.removeCmdListener(CommandID.NONO_ACTIVE_EXP,this.onGetExpHandler);
         SocketConnection.removeCmdListener(CommandID.NONO_ACTIVE_ITEM,this.onGetItemHandler);
      ...
      private function onMusicMCFrameHandler(param1:MouseEvent) : void
      {
         ...
         switch(mc.name)
         {
            case "music_0":
               ...
               {
                  musicResult();
                  SocketConnection.send(CommandID.NONO_ACTIVE_ITEM);
               });
               break;
            case "music_1":
      ...
      {
         AnimateManager.playFullScreenAnimate("resource/bounsMovie/overeffect.swf",function():void
         {
