package com.robot.app.mapProcess
{
   ...
   public class MapProcess_326 extends BaseMapProcess
   {
      ...
      {
         this.initForAll();
         Task748.initTaskForMap326(this,true);
         if(TasksManager.getTaskStatus(TaskController_716.TASK_ID) != TasksManager.COMPLETE)
         {
            Task716.initTaskForMap326(this);
         }
         else if(TasksManager.getTaskStatus(TaskController_748.TASK_ID) != TasksManager.COMPLETE)
         {
            Task748.initTaskForMap326(this);
         }
         else if(TasksManager.getTaskStatus(TaskController_755.TASK_ID) != TasksManager.COMPLETE)
         {
            Task755.initTaskForMap326(this);
         }
         else if(TasksManager.getTaskStatus(TaskController_775.TASK_ID) == TasksManager.ALR_ACCEPT)
         {
            Task775.initTaskForMap326(this);
         }
      }
            ...
            if(conLevel["icon" + _loc2_].hitTestObject(conLevel["Nicon" + _loc3_]))
            {
               if(uint(_loc2_) == _loc3_)
               {
                  ...
                  else
                  {
                     ...
                     (_loc5_ = MapManager.currentMap.libManager.getSound("openStone")).play();
      ...
      private function soudPlay(param1:MouseEvent) : void
      {
         ...
         var _loc4_:Sound;
         (_loc4_ = MapManager.currentMap.libManager.getSound("oneSound" + _loc3_)).play();
      }
