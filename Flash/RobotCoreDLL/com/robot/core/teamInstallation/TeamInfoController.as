package com.robot.core.teamInstallation
{
   ...
   public class TeamInfoController
   {
      ...
      private static function onComHandler(param1:SocketEvent) : void
      {
         ...
         ArmInfo.setFor2967_2965(_info,_loc2_);
         _nexF = FortressItemXMLInfo.getNextForm(_info.id,_info.form);
         var _loc3_:Array = FortressItemXMLInfo.getResIDs(_info.id,_info.form);
         ...
         {
            _loc5_ = 0;
            while(_loc5_ < _info.res.length)
            {
               if(_loc3_[_loc4_] != 0 && _loc3_[_loc4_] == _info.res.getKeys()[_loc5_])
               {
                  _needIdA.push(_loc3_[_loc4_]);
                  _curNumA.push(_info.res.getValues()[_loc5_]);
               }
               _loc5_++;
            }
            _loc4_++;
      ...
      {
         ExchangeOreModel.getData(onDataComHandler);
      }
