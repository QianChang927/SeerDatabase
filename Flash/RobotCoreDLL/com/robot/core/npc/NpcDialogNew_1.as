package com.robot.core.npc
{
   ...
   public class NpcDialogNew_1
   {
      ...
      public static var _HasDanmu:Boolean = false;
      ...
      private static var _curLevel:Sprite;
      ...
      public function NpcDialogNew_1()
      {
         super();
      }
      ...
      public static function setup() : void
      {
         if(_HasDanmu)
         {
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().showBarrage(300,300);
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().showScreen(1000,300);
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().init();
         }
         _bgMc = UIManager.getMovieClip("NPC_BG_MC_NEW_1");
      ...
      public static function show(param1:uint, param2:Array, param3:Array = null, param4:Array = null, param5:Boolean = false, param6:Function = null) : void
      {
         var cls:Class;
         var i1:int = 0;
         var i2:int = 0;
         ...
         if(_HasDanmu)
         {
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().show();
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().playOthersSay();
         }
         ...
         btnBox.removeAll();
         _npcId = npcId;
         _curNpcPath = NPC.getDialogNewNpcHeadPathByID(npcId);
         _curIndex = 0;
         _dialogA = dialogA;
      ...
      private static function addTxtBtn() : void
      {
         ...
         if(_questionA != null)
         {
            ...
            j = 0;
            while(j < _questionA.length)
            {
               if(j == 0)
               {
                  _btnA[j].x = 0;
                  _btnA[j].y = 48;
                  if(_questionA.length == 1)
                  {
                     _btnA[j].setSizeWH(600,40);
                  }
                  else
                  {
                     _btnA[j].setSizeWH(400,40);
                  }
               }
               else if(j == 1)
               {
      ...
      private static function onComHandler(param1:DisplayObject) : void
      {
         ...
         var scale:Number = 386 < mc.width ? 386 / mc.width : 1;
         mc.scaleY = scale;
         mc.scaleX = scale;
         _npcBmp = DisplayUtil.copyDisplayAsBmp(mc);
         _npcBmp.x = 175 - _npcBmp.width + 400;
         _npcBmp.y = -_npcBmp.height + 184;
         _bgMc.addChild(_npcBmp);
         _npcBmp.alpha = 0;
         LevelManager.root.mouseEnabled = false;
         LevelManager.root.mouseChildren = false;
         targetX = 130 - _npcBmp.width;
      ...
      public static function hide() : void
      {
         ...
         if(_HasDanmu)
         {
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().hide();
            getDefinitionByName("com.robot.app2.utilClass.BarrageController").getInstance().stopPlayOthersSay();
         }
         removeEvent();
      ...
      {
         var npcName:String;
         var npcNameParse:ParseDialogStr;
         var str:String;
         ...
         txtBox.removeAll();
         npcName = TasksOfNewStyleXMLInfo.getPetName(_npcId) + "：";
         npcNameParse = new ParseDialogStr(npcName);
         str = "  " + _dialogA[index];
         ...
         npcNameBox.x = 181;
         npcNameBox.y = 45;
         npcNameBox.setSizeWH(600,30);
         npcNameBox.layout = new FlowWarpLayout(FlowWarpLayout.LEFT,FlowWarpLayout.CENTER,-5,-5);
         for each(i in npcNameParse.strArray)
         {
            j = 0;
            while(j < i.length)
            {
               s = i.charAt(j);
               lable = new MLabel(s);
               lable.textColor = uint("0x" + npcNameParse.getColor(count));
               lable.fontSize = 14;
               lable.setSizeWH(20,20);
               lable.cacheAsBitmap = true;
               npcNameBox.append(lable);
               j++;
            }
         }
         txtBox.append(npcNameBox);
      ...
      public static function set curLevel(param1:Sprite) : void
      {
         _curLevel = param1;
      }
      
      public static function get curLevel() : Sprite
      {
         if(_curLevel == null)
         {
