package com.robot.app.mapProcess
{
   ...
   public class MapProcess_348 extends BaseMapProcess
   {
      ...
      private function addYellowArrow(param1:MapEvent) : void
      {
         MapManager.removeEventListener(MapEvent.MAP_OBJECT_INIT_FINISH,this.addYellowArrow);
         CommonUI.addYellowArrowForMapObject("taxiya","controlLevel");
      }
      ...
      {
         FightBossController.fightBoss("哈莫雷特",2);
      }
      ...
      private function onFightBoss(param1:MouseEvent) : void
      {
         if(MapManager.currentMap.id == 348)
         {
            ...
            {
               FightBossController.fightBoss("塔西亚",1);
            }
            if(param1.currentTarget.name == "saiweier")
            {
               FightManager.fightWithBoss("塞维尔",5);
            }
            if(param1.currentTarget.name == "takelin")
            {
               FightBossController.fightBoss("塔克林",0);
            }
         }
      }
