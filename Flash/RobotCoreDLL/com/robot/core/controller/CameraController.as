package com.robot.core.controller
{
   ...
   public class CameraController
   {
      ...
      public static function upDateForTotalAreaPoint(param1:Point) : void
      {
         ...
         var _loc2_:Boolean = _totalArea.containsPoint(param1);
         if(!_loc2_)
         {
            return;
         }
         ...
         {
            _loc3_ = param1.subtract(_viewCenterPoint);
            if(_loc3_.x > _viewArea.x)
            {
               if(_viewArea.right < _totalArea.right)
               {
                  if(_loc3_.x > _totalArea.right - _viewArea.width)
                  {
                     _loc3_.x = _totalArea.right - _viewArea.width;
                  }
                  _viewArea.x = _loc3_.x;
                  _isUpDate = true;
               }
            }
            else if(_viewArea.x > _totalArea.x)
            {
               _viewArea.x = Math.max(_loc3_.x,_totalArea.x);
               _isUpDate = true;
            }
            if(_loc3_.y > _viewArea.y)
            {
               if(_viewArea.bottom < _totalArea.bottom)
               {
                  if(_loc3_.y > _totalArea.bottom - _viewArea.height)
                  {
                     _loc3_.y = _totalArea.bottom - _viewArea.height;
                  }
                  _viewArea.y = _loc3_.y;
                  _isUpDate = true;
               }
            }
            else if(_viewArea.y > _totalArea.y)
            {
               _viewArea.y = Math.max(_loc3_.y,_totalArea.y);
               _isUpDate = true;
            }
         }
      ...
      public static function get cameraMode() : int
      {
         return _cameraMode;
      }
      
      public static function set cameraMode(param1:int) : void
      {
         _cameraMode = param1;
      }
   }
