package com.robot.app.mapProcess
{
   ...
   public class MapProcess_56 extends BaseMapProcess
   {
      ...
      private function onGetCountHandler(param1:SocketEvent) : void
      {
         ...
         this._wendyMc.mouseChildren = false;
         this._wendyMc.mouseEnabled = false;
      }
      ...
      private function onLightClickHandler(param1:MouseEvent) : void
      {
         if(!this._appModel)
         {
            this._appModel = new AppModel(ClientConfig.getGameModule("kickBall/KickBallGame"),"正在进入游戏");
         }
      ...
      private function onWendyClickHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(MainManager.actorModel.nono) && Boolean(MainManager.actorModel.pet))
         {
            ...
            _loc2_ = String(PetXMLInfo.getType(this._petInfo.petID));
            if(_loc2_ == "7")
            {
               ResourceManager.getResource(ClientConfig.getPetSwfPath(this._petInfo.petID,this._petInfo.catchTime),this.onLoadPetHandler);
            }
         }
      }
      ...
      private function onGetExpHandler(param1:SocketEvent) : void
      {
         ...
         var _loc4_:String = String(TextFormatUtil.getRedTxt(String(_loc3_)));
         var _loc5_:String = String(TextFormatUtil.getRedTxt(this._petName));
         if(_loc3_ < 1000)
         {
