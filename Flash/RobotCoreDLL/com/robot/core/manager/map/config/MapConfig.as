   ...
   public class MapConfig
   {
      ...
      public static function configMap(param1:int) : void
      {
         ...
         for each(i in xmllist)
         {
            ...
            if(hitMC == null)
            {
               hitMC = MapManager.currentMap.depthLevel.getChildByName(hit) as MovieClip;
            }
            try
            {
               if(mc != hitMC)
      ...
      public static function getMapPlanetIndex(param1:uint) : int
      {
         var xml:XML = null;
         var mapID:uint = param1;
         xml = XML_DATA.elements("map").(@id == mapID)[0];
      ...
      private static function clickHandler(param1:MouseEvent) : void
      {
         ...
         clickPoint = new Point(_loc3_.x,_loc3_.y);
      ...
      private static function checkHit(param1:Event) : void
      {
         ...
         var pp:Point = MainManager.actorModel.pos;
         if(Point.distance(pp,clickPoint) < 15)
         {
            if(currentComp["fun"] == "")
            {
               if(currentComp["targetID"] == 501 || currentComp["targetID"] == 502)
               {
