package com.robot.core.config.xml
{
   ...
   public class MachXMLInfo
   {
      ...
      private static var _annualExpMap:HashMap = new HashMap();
      
      private static var _annualLinesMap:HashMap = new HashMap();
      ...
      private static function setup() : void
      {
         ...
         _loc1_ = XML(new xmlClass()).elements("annualExp")[0].elements("item");
         for each(_loc2_ in _loc1_)
         {
            _annualExpMap.add(uint(_loc2_.@id),_loc2_);
         }
         _loc1_ = XML(new xmlClass()).elements("annualLines")[0].elements("item");
         for each(_loc2_ in _loc1_)
         {
            _annualLinesMap.add(uint(_loc2_.@id),_loc2_);
         }
      }
      ...
      public static function getExpName(param1:uint) : String
      {
         ...
         if(MainManager.actorInfo.superNono)
         {
            ...
            {
               _loc2_ = _annualExpMap.getValue(param1);
            }
            else
            {
      ...
      public static function getLinesName(param1:uint) : String
      {
         ...
         if(MainManager.actorInfo.superNono)
         {
            if(MainManager.actorInfo.isExtremeNono)
            {
               _loc2_ = _annualLinesMap.getValue(param1);
            }
            else
            {
      ...
      public static function getExpID() : Array
      {
         ...
         if(MainManager.actorInfo.superNono)
         {
            if(MainManager.actorInfo.isExtremeNono)
            {
               xmlArr = _annualExpMap.getValues();
            }
            else
            {
      ...
      public static function getLinesIDForExp(param1:uint, param2:uint, param3:uint) : Array
      {
         ...
         if(MainManager.actorInfo.superNono)
         {
            if(MainManager.actorInfo.isExtremeNono)
            {
               _loc4_ = _annualExpMap.getValue(param1);
            }
            else
            {
         ...
         if(_loc4_)
         {
            _loc6_ = (_loc5_ = String(_loc4_.@lines)).split(",");
            _loc7_ = [];
            for each(_loc8_ in _loc6_)
            {
               if(MainManager.actorInfo.superNono)
               {
                  if(MainManager.actorInfo.isExtremeNono)
                  {
                     _loc4_ = _annualLinesMap.getValue(_loc8_);
                  }
                  else
                  {
               ...
               if(_loc4_)
               {
                  if((_loc9_ = String(_loc4_.@energy)).indexOf(param2.toString()) != -1)
                  {
                     if((_loc10_ = String(_loc4_.@mate)).indexOf(param3.toString()) != -1)
                     {
                        _loc7_.push(_loc8_);
                     }
                  }
