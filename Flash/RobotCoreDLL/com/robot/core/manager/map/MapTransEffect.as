package com.robot.core.manager.map
{
   ...
   public class MapTransEffect extends EventDispatcher
   {
      ...
      private var _theSameMap:Boolean;
      
      public function MapTransEffect(param1:MapModel, param2:int, param3:Boolean = false)
      {
         super();
         this._theSameMap = param3;
      ...
      public function star() : void
      {
         ...
         else
         {
            ...
            (_loc5_ = new Bitmap(_loc4_)).x = _loc1_.x;
            _loc5_.y = _loc1_.y;
            if(this._theSameMap)
            {
               _loc4_.draw(LevelManager.mapLevel.getChildAt(0));
            }
            else
            {
               _loc4_.draw(this._mapModel.root["bg_mc"]);
            }
