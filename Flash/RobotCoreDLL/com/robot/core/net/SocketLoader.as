package com.robot.core.net
{
   ...
   public class SocketLoader extends EventDispatcher
   {
      ...
      private static function onEvent(param1:SocketEvent) : void
      {
         ...
         var _loc2_:Array = _map.getValue(param1.headInfo.cmdID);
         if(_loc2_)
         {
            _loc3_ = _loc2_.shift() as SocketLoader;
            if(_loc2_.length == 0)
            {
            ...
            if(_loc3_)
            {
               _loc3_.dispatchEvent(new SocketEvent(SocketEvent.COMPLETE,param1.headInfo,param1.data));
            }
         }
      }
