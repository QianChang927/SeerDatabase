package com.robot.app.panel
{
   ...
   public class TopSelectPanel extends Sprite
   {
      ...
      public static function set isPlay(param1:Boolean) : void
      {
         ...
         if(_isPlay && _mode == NORMAL)
         {
            StatManager.sendStat2014("巅峰全国联赛结束","打开练习赛面板",StatManager.RUN_ACT);
         }
         else if(_isPlay && _mode == BEYOND)
         {
            StatManager.sendStat2014("巅峰全国联赛结束","打开练习赛突破模式面板",StatManager.RUN_ACT);
         }
         else if(!_isPlay && _mode == NORMAL)
         {
            StatManager.sendStat2014("巅峰全国联赛结束","打开积分赛面板",StatManager.RUN_ACT);
         }
         else if(!_isPlay && _mode == BEYOND)
         {
            StatManager.sendStat2014("巅峰全国联赛结束","打开积分赛突破模式面板",StatManager.RUN_ACT);
         }
      }
      ...
      private static function fightOver(param1:PetFightEvent) : void
      {
         ...
         if((param1.dataObj as FightOverInfo).winnerID != MainManager.actorID)
         {
            if(Math.random() >= 0.5)
            {
               ModuleManager.showModule(ClientConfig.getAppModule("PeakLosePanel"));
            }
         }
      }
