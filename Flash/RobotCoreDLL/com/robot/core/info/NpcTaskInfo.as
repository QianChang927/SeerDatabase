package com.robot.core.info
{
   ...
   public class NpcTaskInfo extends EventDispatcher
   {
      ...
      private var _relateIDs:HashMap;
      ...
      public function checkTaskStatus() : void
      {
         ...
         else
         {
            ...
            while(_loc1_ < TasksManager.taskList.length + 1)
            {
               ...
               {
                  _loc2_ = this.proIDs.indexOf(_loc1_);
         ...
         for each(_loc1_ in this.tasks)
         {
            ...
            {
               _loc2_ = this.tasks.indexOf(_loc1_);
               if(_loc2_ != -1)
               {
      ...
      private function onGetBuff(param1:Array) : void
      {
         ...
         var _loc2_:uint = uint(this.alAcceptIDs[this.alAcceptIndex]);
         param1.pop();
         ...
         if(_loc3_ && _loc2_ != 25)
         {
            this.canCompleteIDs.push(_loc2_);
         }
         ++this.alAcceptIndex;
         this.checkAlrAcceptProStatus();
      }
      ...
      {
         DebugTrace.show("-------------------Task Info::yellow excal mark");
      ...
      {
         DebugTrace.show("-------------------Task Info::yellow question mark");
         if(!this.isQueue || this.queueNum == 0)
         {
      ...
      {
         DebugTrace.show("-------------------Task Info::blue question mark");
         if(!this.isQueue || this.queueNum == 0)
         {
      ...
      {
         var _loc2_:uint = 0;
         var _loc1_:Array = this._acceptList.slice();
         for each(_loc2_ in this.proIDs)
         {
            if(TasksManager.getTaskStatus(_loc2_) == TasksManager.ALR_ACCEPT)
            {
               if(_loc2_ == 1 || _loc2_ == 2 || _loc2_ == 3 || _loc2_ == 4)
               {
                  _loc2_ = 4;
               }
