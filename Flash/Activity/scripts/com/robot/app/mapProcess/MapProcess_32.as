package com.robot.app.mapProcess
{
   ...
   public class MapProcess_32 extends BaseMapProcess
   {
      ...
      override protected function init() : void
      {
         ...
         conLevel.addChild(this._seerModel);
         ...
         getDefinitionByName("com.robot.app2.mapProcess.control.ASignalThatNeverDisappearsController").initMap32(this);
         if(TasksManager.getTaskStatus(130) == TasksManager.COMPLETE)
         {
      ...
      private function update() : void
      {
         ...
         {
            if(KTool.getBit(param1[0],22) > 0)
            {
               conLevel["mapBtn_1"].visible = false;
            }
            else
            {
               conLevel["mapBtn_1"].visible = true;
            }
            if(KTool.getBit(param1[0],23) > 0)
            {
               conLevel["mapBtn_2"].visible = false;
            }
            else
            {
               conLevel["mapBtn_2"].visible = true;
            }
         });
      }
      ...
      private function onConclickHandle(param1:MouseEvent) : void
      {
         ...
         {
            case "mapBtn_" + index:
               MapStoryManager.dialog(172,19 + index,function(param1:int):void
               {
                  var va:int = param1;
                  SocketConnection.sendByQueue(41900,[103,21 + index],function(param1:*):void
                  {
                     update();
                  });
               });
      ...
      override public function destroy() : void
      {
         ...
         getDefinitionByName("com.robot.app2.mapProcess.control.ASignalThatNeverDisappearsController").destroyMap32();
      }
